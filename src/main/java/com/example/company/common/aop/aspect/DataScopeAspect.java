...
public class DataScopeAspect {
    ...
    @Pointcut("@annotation(com.example.company.common.aop.annotation.DataScope)")
    ...
    protected void handleDataScope(final JoinPoint joinPoint) {
        ...
        List<Role> roles = roleService.getRoleInfoByUserId(id);
        if (CollectionUtils.isEmpty(roles) || roles.size() == 0) {
            return;
        }
        //角色未配置数据权限范围, 那么不限制
        List<Role> list = roles.parallelStream().filter(
                one -> null != one.getDataScope()).collect(Collectors.toList());
        if (CollectionUtils.isEmpty(list) || list.size() == 0) {
            return;
        }
        //如果存在某角色配置了全部范围， 那么不限制
        if (list.stream().anyMatch(Role -> Constant.DATA_SCOPE_ALL.equals(
                Role.getDataScope()))) {
            return;
        }
        //获取绑定的人
     ...
     * @return 用户id集合
     */
    private List<String> getUserIdsByRoles(List<Role> sysRoles, String userId) {
        //本人
        User sysUser = userService.getById(userId);
        //本部门
        Dept sysDept = deptService.getById(sysUser.getDeptId());
        ...
        Map<Integer, List<Role>> dataScopeMap =
                sysRoles.parallelStream().collect(
                        Collectors.groupingBy(Role::getDataScope));
        dataScopeMap.forEach((k, v) -> {
            if (Constant.DATA_SCOPE_CUSTOM.equals(k)) {
                //自定义
                //根据角色id，获取所有自定义关联的部门id
                QueryWrapper<RoleDept> queryWrapper =
                        Wrappers.<RoleDept>query().select("dept_id").in("role_id",
                                v.parallelStream().map(Role::getId).collect(
                                        Collectors.toList()));
                deptList.addAll(roleDeptService.listObjs(queryWrapper));
            } else if (Constant.DATA_SCOPE_DEPT_AND_CHILD.equals(k)) {
                //本部门及以下
                if (sysDept != null && StringUtils.isNotBlank(sysDept.getDeptNo())) {
                    //获取本部门以下所有关联的部门
                    QueryWrapper<Dept> queryWrapper =
                            Wrappers.<Dept>query().select("id").like(
                                    "relation_code", sysDept.getDeptNo());
        ...
        if (!CollectionUtils.isEmpty(deptList)) {
            QueryWrapper<User> queryWrapper =
                    Wrappers.<User>query().select("id").in("dept_id", deptList);
        ...
        return userIdList.parallelStream().map(Object::toString).collect(
                Collectors.toList());
    }
}
