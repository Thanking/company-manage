...
public class PermissionController {
    ...
    public DataResult addPermission(@RequestBody @Valid Permission vo) {
        verifyFormPid(vo);
        vo.setStatus(1);
    ...
    @RequiresPermissions(":permission:deleted")
    ...
    public DataResult updatePermission(@RequestBody @Valid Permission vo) {
        if (StringUtils.isEmpty(vo.getId())) {
            return DataResult.fail("id不能为空");
        }
        Permission permission = permissionService.getById(vo.getId());
        if (null == permission) {
            throw new BusinessException(BaseResponseCode.DATA_ERROR);
        }
        // 只有类型变更或者所属菜单变更
        if (permission.getType().equals(vo.getType()) || !permission.getPid().equals(vo.getPid())) {
            verifyFormPid(vo);
        }
    ...
    private void verifyFormPid(Permission permission) {
        Permission parent;
        parent = permissionService.getById(permission.getPid());
        switch (permission.getType()) {
            case 1:
                ...
                } else if (!"0".equals(permission.getPid())) {
                    throw new BusinessException(BaseResponseCode.OPERATION_MENU_PERMISSION_CATALOG_ERROR);
                }
                break;
            case 2:
